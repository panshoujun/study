...
namespace WeixinApi.Filters
{
    ...
    /// Swagger区域文档过滤器
    /// </summary>
    public class SwaggerAreasSupportDocumentFilter1:IDocumentFilter
    {
        /// <summary>
        /// 申请处理
        /// </summary>
        ...
        public void Apply(SwaggerDocument swaggerDoc, SchemaRegistry schemaRegistry, IApiExplorer apiExplorer)
        {
            ...
            foreach (var item in swaggerDoc.paths)
            {
                ...
                if (keys[2].IndexOf('.')!=-1)
                {
                    ...
                    {
                        string controllerName = directoryNames[nplen + 2];
                        replacePaths.Add(
        ...
        /// <param name="schemaRegistry"></param>
        /// <param name="apiExplorer"></param>
        public void Apply2(SwaggerDocument swaggerDoc, SchemaRegistry schemaRegistry, IApiExplorer apiExplorer)
        {
            IDictionary<string, PathItem> replacePaths = new ConcurrentDictionary<string, PathItem>();
            foreach (var item in swaggerDoc.paths)
            {
