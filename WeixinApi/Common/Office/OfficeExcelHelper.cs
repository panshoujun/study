...
namespace Common
{
    public class OfficeExcelHelper
    {
        ...
        //将内存中数据表格插入到Microsoft.Office.Interop.Excel指定工作表的指定位置二
        {
            ...
            for (int j = 0; j <= dt.Columns.Count - 1; j++)
            {
                ((Range)ws.Rows[1, Missing.Value]).EntireColumn.AutoFit();
                ((Range)ws.Rows[1, Missing.Value]).EntireColumn.HorizontalAlignment = XlVAlign.xlVAlignCenter;
            }
            //ws.get_Range("A1").EntireColumn.AutoFit();
            //ws.get_Range("A1").EntireColumn.HorizontalAlignment = XlVAlign.xlVAlignCenter;
            //ws.get_Range("B1").EntireColumn.AutoFit();
            //ws.get_Range("B1").EntireColumn.HorizontalAlignment = XlVAlign.xlVAlignCenter;
        }
