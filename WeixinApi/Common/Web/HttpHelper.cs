...
namespace Common
{
    public class HttpHelper
    {
        ...
        /// <returns></returns>
        public string GetHtmlContext(string url)
        {
            string result;
            ...
            try
            {
                ...
                {
                    result = response.StatusDescription.ToString();
                }
                else
                {
        ...
        public string PostWebRequest(string postUrl, string paramData, Encoding dataEncode, bool IsXml = true)
        {
            ...
            try
            {
                ...
                if (IsXml)
                {
                    request.ContentType = "application/xml";  //  "application/json";     
                }
                else
                {
        ...
        /// <returns></returns>
        public static string PostRequest(string url, string postData, string contentType = null, Cookie cookie = null)
        {
            try
            {
        ...
        /// <param name="url">URI</param>
        /// <returns></returns>
        public static bool GetRequest(string url, string filePath, ref string mg)
        {
            try
            {
                HttpWebRequest httpWebRequest = (HttpWebRequest)WebRequest.Create(url);
        ...
        public static string UploadRequest(string url, string fileNamePath, string saveName)
        {
            // 要上传的文件
            try
            {
                ...
                return UploadRequest(url, fs, saveName);
            }
            catch (Exception ex)
            {
                return "";
        ...
        /// <param name="saveName">文件上传后的名称</param>
        /// <returns>成功返回1，失败返回0</returns> 
        public static string UploadRequest(string url, Stream fileStream, string saveName,Dictionary<string,string> param=null)
        {
            ...
            // 写入字符串的Key
            string stringKeyHeader = "--" + strBoundary + "\r\nContent-Disposition: form-data; name=\"{0}\"" + "\r\n\r\n{1}\r\n";
            if (param != null && param.Count() > 0)
            {
                foreach (var key in param.Keys)
                {
                    sb.Append(string.Format(stringKeyHeader, key, param[key]));
                }
            }
            sb.Append("--");
            sb.Append(strBoundary);
            sb.Append("\r\n");
            sb.Append("Content-Disposition: form-data; token=123; name=\"");
            sb.Append("file");
            sb.Append("\"; filename=\"");
            sb.Append(saveName);
            sb.Append("\"");
        ...
        /// <returns></returns>
        public static string UploadRequest(string url, string filePath, string filename, NameValueCollection stringDict,Stream fileStreams, string fileKeyName = "file", int timeOut = 900000)
        {
